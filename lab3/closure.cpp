...
void add_closure(char, LR0Item& iem, ExtendedGrammar&);
void getLR0Items(vector<LR0Item>&, ExtendedGrammar&, int&, gotoMap&);
    ...
    while (!input.eof())
    {
        ...
        if (isupper(val))
        {
            ...
            if (nonTerminal.empty())
            {
                ...
                string temp = ".";
                temp.push_back(val);
                lr0items[0].Push(new ExtendedProduction('\'', temp));
                printf("'->%c\n", val);
            ...
            if (production[val] == "")
            {
                ...
                if (isTerminal(rhs[0]) == false)
                    lr0items[0].Push(new ExtendedProduction(val, "." + rhs));
            }
            else
            {
                production[val] += "|" + line.substr(3, line.length());
...
void add_closure(char lookahead, LR0Item& item, ExtendedGrammar& grammar)
{
...
void getLR0Items(vector<LR0Item>& lr0items, ExtendedGrammar& grammar, int& itemid, gotoMap& globalGoto)
{
    printf("I%d:\n", itemid);
    ...
    for (int i = 0; i < lr0items[itemid].Size(); i++)
    {
        ...
        if (lr0items[itemid].gotos.find(lookahead) == lr0items[itemid].gotos.end())
        {
            ...
            if (globalGoto.find(production) == globalGoto.end())
            {
                ...
                lr0items.back().Push(new ExtendedProduction(lhs, newRhs));
        ...
        else
        {
            ...
            if (!lr0items[nextItem].Contains(string(&lhs, 1) + "->" + rhs))
                lr0items[nextItem].Push(new ExtendedProduction(lhs, rhs));
            swap(rhs[at], rhs[at + 1]);
            printf("\t%-20s\n", &production[0]);
        }
    }
